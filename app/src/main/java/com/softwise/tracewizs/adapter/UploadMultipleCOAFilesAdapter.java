...
public class UploadMultipleCOAFilesAdapter extends RecyclerView.Adapter<UploadMultiFileViewHolder> {
    List<UploadFiles> mUploadFilesList = new ArrayList<>();
    OnCOAFileSelectListeners mOnFileSelectListeners;
    private Context mContext;

    public UploadMultipleCOAFilesAdapter(Context context, List<UploadFiles> uploadFiles, OnCOAFileSelectListeners fileSelectListeners) {
        this.mContext = context;
        this.mUploadFilesList = uploadFiles;
        this.mOnFileSelectListeners = fileSelectListeners;
    }
    public UploadMultipleCOAFilesAdapter(Context context, List<UploadFiles> uploadFiles) {
        this.mContext = context;
        this.mUploadFilesList = uploadFiles;
    }
    ...
    public void onBindViewHolder(@NotNull UploadMultiFileViewHolder holder, int position) {
        ...
        holder.imgFileRemove.setOnClickListener(v -> {
            mOnFileSelectListeners.coadeleteFile(uploadFiles,position);
        });
    }
    ...
    public interface OnCOAFileSelectListeners {
        void coafileSelect(UploadFiles uploadFiles);
        void coadeleteFile(UploadFiles uploadFiles,int pos);
    }
